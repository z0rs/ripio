...
/**
 * @title LimitedMinter
 * @notice A contract that enforces daily minting limits for multiple tokens
 * @dev This contract allows admins of tokens to register their tokens and set daily minting limits.
...
/**
 * @notice Interface for interacting with tokens
 * @dev Minimal interface required for LimitedMinter to interact with tokens
 */
interface IToken {
    function hasRole(bytes32 role, address account) external view returns (bool);
...
contract LimitedMinter is AccessControlEnumerable, ReentrancyGuard, Pausable {
    ...
    modifier onlyExternalAdmin(address token) {
        if (!IToken(token).hasRole(IToken(token).DEFAULT_ADMIN_ROLE(), msg.sender)) {
